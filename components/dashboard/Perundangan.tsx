...
export default function Perundangan() {
  ...
  useEffect(() => {
    fetch("/api/katalog?sheet=petisyen&limit=5000")
      .then((r) => r.json())
  ...
  const years = useMemo(() => Array.from(new Set(data.map((r) => String(r["TAHUN PILIHAN RAYA"] || "").trim()))).filter(Boolean).sort().reverse(), [data]);
  const negeris = useMemo(() => Array.from(new Set(data.map((r) => String(r["NEGERI"] || "").trim()))).filter(Boolean).sort(), [data]);
  ...
  const stats = useMemo(() => ({
    jumlah: filtered.reduce((s, r) => s + toNum(r["BIL PETISYEN"]), 0),
    pr: new Set(filtered.map((r) => String(r["PILIHAN RAYA"] || "").trim())).size,
    mahkamah: Array.from(new Set(filtered.map((r) => String(r["MAHKAMAH"] || "").trim()))).filter(Boolean).length,
  }), [filtered]);

  // Chart: aggregate by PILIHAN RAYA
  const chartAgg = useMemo(() => {
    const map = new Map<string, number>();
    for (const r of data) {
      const pr = String(r["PILIHAN RAYA"] || "").trim();
      if (!pr) continue;
      map.set(pr, (map.get(pr) || 0) + toNum(r["BIL PETISYEN"]));
    }
  ...
  return (
    <div>
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: "Jumlah Petisyen", value: stats.jumlah.toLocaleString(), color: "#3B82F6" },
          { label: "Pilihan Raya", value: String(stats.pr), color: "#F97316" },
          { label: "Mahkamah", value: String(stats.mahkamah), color: "#581CDC" },
        ].map((s) => (
      ...
      {chartAgg.length > 0 && (
        <div className="bg-white border border-spr-border rounded-xl p-6 mb-6">
          <h3 className="text-lg font-semibold text-spr-navy mb-4">Bilangan Petisyen Mengikut Pilihan Raya</h3>
          <div style={{ height: chartAgg.length * 36 + 40 }}>
            <Bar
              data={{
                labels: chartAgg.map(([pr]) => pr),
                datasets: [{ data: chartAgg.map(([, v]) => v), backgroundColor: "#3B82F6", borderRadius: 4, barThickness: 24 }],
              }}
              options={{
                indexAxis: "y", responsive: true, maintainAspectRatio: false,
                plugins: { legend: { display: false }, tooltip: { backgroundColor: "#fff", titleColor: "#1A1A2E", bodyColor: "#5A5A72", borderColor: "#E2E2EA", borderWidth: 1, padding: 10, cornerRadius: 8 }, datalabels: { anchor: "end", align: "right", offset: 6, color: "#1A1A2E", font: { size: 11, weight: "bold" } } },
                scales: { x: { grid: { display: false }, ticks: { font: { size: 11 }, color: "#8A8AA0" }, max: Math.max(...chartAgg.map(([, v]) => v)) * 1.2 }, y: { grid: { display: false }, ticks: { font: { size: 10 }, color: "#1A1A2E" } } },
              }}
            />
          </div>
        </div>
      ...
      <div className="bg-white border border-spr-border rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-spr-border-light">
          <h3 className="text-base font-semibold text-spr-navy">Senarai Petisyen</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead><tr className="bg-[#1a2332]">
              {["PILIHAN RAYA", "NEGERI", "PARLIMEN", "DUN", "MAHKAMAH", "BIL PETISYEN"].map((h) => (
            ...
            <tbody>
              {pageData.map((r, i) => (
                <tr key={i} className={`border-t border-spr-border-light hover:bg-spr-primary-50 ${i % 2 === 0 ? "bg-white" : "bg-[#F8FAFC]"}`}>
                  ...
                  <td className="px-4 py-2 text-spr-text">{String(r["MAHKAMAH"] || "").trim()}</td>
                  <td className="px-4 py-2 text-spr-text font-medium">{toNum(r["BIL PETISYEN"])}</td>
                </tr>
              ))}
              {pageData.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-spr-text-muted">Tiada data.</td></tr>}
            </tbody>
          </table>
        </div>
        {totalPages > 1 && (
